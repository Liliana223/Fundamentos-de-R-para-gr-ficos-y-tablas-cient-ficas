...
grafico_base +
  annotate("text", x = 5.5, y = 8, label = "Tendencia general", size = 6)
...
grafico_base +
  geom_vline(xintercept = 5.5, linetype = 1, color = 1)

grafico_base +
  geom_segment(x = 5, y = 4,
               xend = 7, yend = 6.5,
               color = 1,
               arrow = arrow())
...
grafico_final <- ggplot(datos, aes(x = longitud_sepalo, y = longitud_petalo, color = especie)) +
  ...
  theme( # Otros elementos
    plot.title = element_text(
      ...
      vjust = 1          # Ajuste vertical. Mueve el texto de arriba hacia abajo
    )
  )

# hjust: Valor	Resultado
#        0    	Izquierda
#       0.5	    Centro
#        1    	Derecha
...
ggsave(
  "grafico_publicacion.png", # Puedes cambiar la extensión: TIFF, PDF
  plot = grafico_final,
